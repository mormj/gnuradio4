...
void runTest() {
    ...
    for (std::size_t i = 0; i < nPorts; i++) {
        ...
        expect(graph.connect(*sources[i], "out"s, perfBlock, "inputs#"s + std::to_string(i)).has_value());
    }
    ...
    for (std::size_t i = 0; i < nPorts; i++) {
        ...
        expect(graph.connect(perfBlock, "outputs#"s + std::to_string(i), *sinks[i], "in"s).has_value());
    }
...
void runTestPureCopy() {
    ...
    for (std::size_t i = 0; i < nPorts; i++) {
        ...
        expect(graph.connect(*sources[i], "out"s, *copies[i], "in"s).has_value());
        expect(graph.connect(*copies[i], "out"s, *sinks[i], "in"s).has_value());
    }
